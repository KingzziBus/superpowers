...
**目标：** 通过一个原生 JavaScript 插件为 OpenCode.ai 添加完整的 superpowers 支持，该插件与现有 Codex 实现共享核心功能。

**架构：** 将通用技能发现/解析逻辑提取到 `lib/skills-core.js`，重构 Codex 以使用它，然后使用其原生插件 API 构建 OpenCode 插件（含自定义工具和会话钩子）。
...
**步骤 1：创建 lib/skills-core.js（带 extractFrontmatter 函数）**
...
在文件顶部现有 require 语句之后（第 6 行附近），添加：
...
将调用从 `findSkillsInDir(` 改为 `skillsCore.findSkillsInDir(`
...
将调用从 `checkForUpdates(` 改为 `skillsCore.checkForUpdates(`
...
在 use_skill 工具定义之后、关闭 tools 数组之前，添加：
...
找到文件中关于支持平台的部分（搜索 "Codex"），并在其后添加：
...
运行：`tree -L 2 .opencode/`（或 `find .opencode -type f`如果没有 tree）
...
创建完成总结，显示：
- 提交总数
- 已创建文件：`lib/skills-core.js`、`.opencode/plugin/superpowers.js`、`.opencode/INSTALL.md`
- 已修改文件：`.codex/superpowers-codex`、`README.md`、`RELEASE-NOTES.md`
- 执行的测试：Codex 命令已验证
...
以下步骤需要已安装 OpenCode，不属于自动实施的一部分：
