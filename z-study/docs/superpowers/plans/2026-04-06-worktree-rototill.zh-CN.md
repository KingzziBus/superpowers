...
**目标：** 让 Superpowers 在宿主平台提供原生 worktree 系统时优先使用它们，在没有时回退到手动 git worktree，并修复三个已知的完成阶段 bug。

**架构：** 重写两个技能文件（`using-git-worktrees`、`finishing-a-development-branch`），对三个文件做单行集成更新（`executing-plans`、`subagent-driven-development`、`writing-plans`）。核心变更是增加环境检测（`GIT_DIR != GIT_COMMON`）和"原生工具优先"的创建路径。这些是 Markdown 技能指令文件，不是应用程序代码——"测试"是使用 `testing-skills-with-subagents` TDD 框架的智能体行为测试。

**技术栈：** Markdown（技能文件）、Bash（测试脚本）、Claude Code CLI（`claude -p` 用于无头测试）

**Spec（规格）：** `docs/superpowers/specs/2026-04-06-worktree-rototill-design.md`

---

### Task 1：门控 —— TDD 验证 Step 1a（原生工具偏好）

Step 1a 是整个设计承重的核心假设。如果智能体不优先使用原生 worktree 工具而非 `git worktree add`，则 spec 失败。**在改动任何技能文件之前**，先验证这一点。

**文件：**
- 新建：`tests/claude-code/test-worktree-native-preference.sh`
...
**本任务是门控（gate）。** 如果在 2 轮 REFACTOR 后 GREEN 阶段失败，立即停止。不要进入 Task 2。回报——创建方法需要重新设计。

- [ ] **第 1 步：编写 RED 基线测试脚本**

创建测试脚本，将分别运行"无"和"有"更新后技能文本的场景。RED 阶段针对当前技能（没有 Step 1a）运行。
...
- [ ] **第 2 步：运行 RED 阶段 —— 确认智能体当前使用 `git worktree add`**

执行：`cd tests/claude-code && bash test-worktree-native-preference.sh red`

预期：`[RED CONFIRMED] Agent did NOT use EnterWorktree`——智能体使用 `git worktree add`，因为当前技能没有原生工具偏好。

原样记录智能体的确切输出和任何合理化（rationalization）理由。这是技能必须修复的基线失败。

- [ ] **第 3 步：如果 RED 确认通过，继续。编写 Step 1a 技能文本。**

创建技能的临时测试版本，**只**增加 Step 1a（最小化改动以隔离变量）。将该小节添加到技能的创建指令顶部，**在**既有的目录选择过程之前：
...
- [ ] **第 4 步：运行 GREEN 阶段 —— 确认智能体现在使用 EnterWorktree**

执行：`cd tests/claude-code && bash test-worktree-native-preference.sh green`

预期：`[PASS] Agent used native EnterWorktree tool`

如果失败：原样记录智能体的确切输出和合理化理由。这是 REFACTOR 信号——Step 1a 文本需要修订。最多尝试 2 轮 REFACTOR。如果 2 轮后仍失败，停止并回报。

- [ ] **第 5 步：运行 PRESSURE 阶段 —— 确认智能体在压力下抵抗回退**

执行：`cd tests/claude-code && bash test-worktree-native-preference.sh pressure`

预期：`[PASS] Agent used native tool even under time pressure`

如果失败：原样记录合理化理由。在 Step 1a 文本中添加显式反制措施（例如 Red Flag 条目："Never use git worktree add when your platform provides a native worktree tool"）。重新运行。
...
### Task 2：重写 `using-git-worktrees` 的 SKILL.md
...
执行：`wc -l skills/using-git-worktrees/SKILL.md`

预期：约 200-220 行。扫描是否有 Markdown 格式问题。
...
### Task 3：重写 `finishing-a-development-branch` 的 SKILL.md

完整重写完成技能。增加环境检测、修复三个 bug、增加基于来源（provenance）的清理逻辑。
...
执行：`wc -l skills/finishing-a-development-branch/SKILL.md`
...
对引用 `using-git-worktrees` 的三个文件做单行修改。
...
- [ ] **第 2 步：更新 subagent-driven-development 集成行**
...
执行：`cd tests/claude-code && bash run-skill-tests.sh`

预期：所有既有测试通过。如果有失败，调查——Task 4 的集成修改可能破坏了某个内容断言。

- [ ] **第 2 步：重新运行 Step 1a 的 GREEN 测试**

执行：`cd tests/claude-code && bash test-worktree-native-preference.sh green`

预期：通过——智能体在使用最终技能文本（而非仅 Task 1 的最小 Step 1a 增量）时仍使用 EnterWorktree。

- [ ] **第 3 步：手动验证 —— 端到端阅读两个重写后的技能**

完整阅读 `skills/using-git-worktrees/SKILL.md` 和 `skills/finishing-a-development-branch/SKILL.md`。检查：

1. 没有引用旧行为（硬编码的 `CLAUDE.md`、交互式目录提示、"REQUIRED" 语言）
2. 步骤编号在每个文件内保持一致
3. Quick Reference 表格与正文一致
4. Integration 小节正确交叉引用
5. 无 Markdown 格式问题
